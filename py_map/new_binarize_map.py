...
def do_depth_analysis(wrk_pth,ref_pth,trg_pth,
                      cur_mod,rows,cols,
                      baseline,fmu,wall,
                      ptch_s,srch_s,
                      out_abs,out_rel,is_wall,trg_wall=0):
    run_cmd=wrk_pth+' '+ref_pth+' '+trg_pth+' '\
            +('%d '%cur_mod)+('%d '%rows)\
            +('%d '%cols)+('%f '%fmu)\
            +('%d '%baseline)+('%d '%wall)\
            +('%d '%srch_s)+('%d '%ptch_s)
    run_cmd='..\\x64\\Release\\new_binarize.exe '+run_cmd
    print(run_cmd)
    os.system(run_cmd)
    def mk_spk_hisgram(file_str1,file_str2,title_str,out_str):
        data1=np.loadtxt(file_str1)
        data2=np.loadtxt(file_str2)
        plt.cla()
        plt.clf()
        plt.title(title_str)
        plt.hist(data1,bins=50,histtype='bar',alpha=0.5,label='old')
        plt.hist(data2,bins=50,histtype='bar',alpha=0.5,label='new')
        plt.xlabel('spackle area')
        plt.grid(True)
        plt.legend()
        plt.savefig(out_str)
        return
    mk_spk_hisgram(wrk_pth+'\\res\\cur_set.txt',
                   wrk_pth+'\\res\\cnb_set.txt',
                   '',
                   wrk_pth+'\\res\\spackle_area.png')
    sm_gray_str=wrk_pth+'\\res\\small_gray.txt'
    sm_gray=np.loadtxt(sm_gray_str,dtype=np.int32)
    th_gray_str=wrk_pth+'\\res\\other_gray.txt'
    th_gray=np.loadtxt(th_gray_str,dtype=np.int32)
    def mk_gray_hisgram():
        plt.cla()
        plt.clf()
        plt.xticks(range(0,255,10),rotation=60)
        plt.hist(sm_gray,bins=256,range=(0,255),
                 histtype='bar',alpha=0.5,label='area<=5')
        plt.hist(th_gray,bins=256,range=(0,255),
                 histtype='bar',alpha=0.5,label='other')
        plt.xlabel('gray level')
        plt.grid(True)
        plt.legend()
        plt.savefig(wrk_pth+'\\res\\small_gray.png')
        return
    mk_gray_hisgram()
    depth_old=np.loadtxt(wrk_pth+'/res/soft_depth2.txt',dtype=np.float32)
    depth_new=np.loadtxt(wrk_pth+'/res/soft_depth3.txt',dtype=np.float32)
...
if __name__=='__main__':
    ...
    print('trg_wall=',trg_wall)
    do_depth_analysis(wrk_pth,ref_pth,trg_pth,
                      cur_mod,rows,cols,
                      baseline,fmu,wall,
                      ptch_s,srch_s,
                      out_abs,out_rel,is_wall,trg_wall)
