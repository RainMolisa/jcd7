...
namespace ofst
{
	...
	static const int minNum1Rec = 0;// 32;  //if num 1 in a receive block is smaller than this, deems as invalid
	static const int goodMatchThres = 4;// 0.5;// 0.5;
		...
		bool is_subpixel=true)  /*øÈ∆•≈‰*/
	{
		...
		for (int y = 0; y < ref.rows; y++)
		{
			...
			{
				if (ref.at<uchar>(y, x) > 0)
					ref.at<uchar>(y, x) = 1;
				else
					ref.at<uchar>(y, x) = 0;
				//
				if (rel.at<uchar>(y, x) > 0)
					rel.at<uchar>(y, x) = 1;
				else
					rel.at<uchar>(y, x) = 0;
			}
	...
	int16_t* fastBlockMatchPadding2(Mat& ref, Mat& rel, \
		Mat* peakMatch=NULL, Mat* subpixeMap = NULL,\
		Mat* score1 = NULL, Mat* max_ix = NULL, \
		bool is_subpixel = true)  /*øÈ∆•≈‰*/
	{
		//const int mbsize = 19;// 23;// 19;	//search window size
		for (int y = 0; y < ref.rows; y++)
		...
		*peakMatch = Mat(rel.rows, rel.cols, CV_32SC1, Scalar::all(0));
		...
		for (int i = 0; i < rel.rows; i++)
		{
			for (int j = 0; j < rel.cols; j++)
			{
				if ((peakMatch->at<int>(i, j) >= num1Count[i * width + j] / goodMatchThres) && 
					(peakMatch->at<int>(i, j) > minNum1Rec))
				{
					if (is_subpixel)
					{
						index = (i + mbsize - 1) * width1 + j + mbsize - 1;
